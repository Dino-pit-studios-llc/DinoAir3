...
def _secure_write_text(path: str, content: str) -> None:
    ...
    try:
        ...
        try:
            ...
            # Best-effort on non-POSIX; ignore chmod failures
            pass
    except Exception:
        try:
            OS.close(fd)
...
class DeepSourceCoverageSetup:
    ...
    @staticmethod
    def install_deepsource_cli():
        """Ensure DeepSource CLI is present without using insecure shell pipelines."""
        try:
            result = safe_run(
                ["deepsource", "--version"],
                allowed_binaries={"deepsource", "deepsource.exe"},
                timeout=15,
                check=False,
                text=True,
            )
        ...
        print("⚠️  Skipping automatic install for security (no shell pipes).")
        print("   Please install manually via official instructions:")
        print("   https://deepsource.io/cli")
        return False

    @staticmethod
    def check_test_coverage_tools():
...
"""
        ...
        _secure_write_text(str(config_file), coverage_config)
...
def report_to_deepsource():
    ...
    try:
        ...
        print("💡 Install from: https://deepsource.io/cli (avoid shell pipes)")
        return False
...
"""
    ...
    def run_setup(self):
        ...
        # Install CLI
        if not DeepSourceCoverageSetup.install_deepsource_cli():
