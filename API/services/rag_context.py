...
class RagContextService:
    ...
    def get_context(
        ...
        method, unavailable = RagContextService._load_context_method()
        if unavailable:
            return resp(False, None, RAG_UNAVAILABLE_MSG, 501)
        if method is None:
        ...
        call_args = RagContextService._filtered_kwargs(method, kwargs)

        try:
            data: Any = method(**call_args)
            keys_preview = RagContextService._preview_dict_keys(data, limit=10)
            log.debug(
    ...
    # -------------------------
    @staticmethod
    def _load_context_method() -> tuple[Callable[..., Any] | None, bool]:
        try:
