...
- invoke(service_desc, payload) posts to {base}/v1/chat/completions with:
  { "model": <model>, "messages": payload["messages"], "options": payload.get("options") }
...
class LMStudioAdapter(ServiceAdapter):
    ...
    def _prepare_request_body(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        body["messages"] = LMStudioAdapter._validate_messages(payload.get("messages"))
    ...
    def _make_request_with_retries(
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                last_exc = exc
                self._handle_network_error(exc, attempt, attempts)
            except RetryableError:
                self._handle_retryable_error(attempt, attempts)
            except httpx.RequestError as exc:
                last_exc = exc
                self._handle_request_error(exc, attempt, attempts)
    ...
    def _handle_network_error(
        ...
        Raises:
            AdapterError: If retries exhausted
        """
        if attempt < attempts:
            self._sleep_backoff(attempt)
        else:
            LMStudioAdapter._raise_network_error(exc)

    @staticmethod
    def _handle_retryable_error(attempt: int, attempts: int) -> None:
    ...
    def _attempt_request(
        ...
        if 200 <= resp.status_code < 300:
            return LMStudioAdapter._parse_successful_response(resp)
    ...
    @staticmethod
    def _raise_fallback_error(last_exc: Exception | None) -> None:
