...
class ProfanityFilter:
    ...
    def detect_profanity(self, text: str) -> list[ProfanityMatch]:
        ...
        Args:
            text: Text to scan for profanity

        Returns:
            List of ProfanityMatch objects for detected profanity
        """
        if not text:
            return []
        ...
        for word in words:
            ...
            word_pos = text.lower().find(word.lower(), position)
            if word_pos == -1:
                continue

            # Check each severity level for matches
            for severity, word_set in self.word_lists.items():
                if word.lower() in word_set:
                    # Get context to check for false positives
                    context = self._get_context(text, word_pos, len(word))

                    # Skip if it's a technical term in technical context
                    if not self._is_technical_term(word, context):
                        matches.append(
                            ProfanityMatch(
                                word=word.lower(),
                                position=word_pos,
                    ...
                    break  # Found in this severity level, no need to check others
    ...
    def filter_profanity(self, text: str, min_severity: Severity = Severity.mild) -> FilterResult:
        ...
        score = ProfanityFilter._calculate_score(filtered_matches, len(text))
    ...
    @staticmethod
    def _calculate_score(matches: list[ProfanityMatch], text_length: int) -> float:
...
def filter_profanity(text: str, min_severity: Severity = Severity.mild) -> str:
    ...
    result = profanity_filter.filter(text, min_severity)
