...
class PatternNormalizer:
    ...
    # Time unit replacement constants
    _REPLACEMENT_MINUTES = r"\1 minutes"
    _REPLACEMENT_HOURS = r"\1 hours"
    _REPLACEMENT_SECONDS = r"\1 seconds"

    def __init__(self):
        """Initialize with pattern replacements and shortcuts."""
        # Common time pattern replacements
        self.time_patterns: dict[str, str] = {
            r"\b(\d+)\s*mins?\b": self._REPLACEMENT_MINUTES,
            r"\b(\d+)\s*min\b": self._REPLACEMENT_MINUTES,
            r"\b(\d+)\s*hrs?\b": self._REPLACEMENT_HOURS,
            r"\b(\d+)\s*hr\b": self._REPLACEMENT_HOURS,
            r"\b(\d+)\s*secs?\b": self._REPLACEMENT_SECONDS,
            r"\b(\d+)\s*sec\b": self._REPLACEMENT_SECONDS,
            r"\b(\d+)\s*ms\b": r"\1 milliseconds",
            r"\b(\d+)m\b": self._REPLACEMENT_MINUTES,
            r"\b(\d+)h\b": self._REPLACEMENT_HOURS,
            r"\b(\d+)s\b": self._REPLACEMENT_SECONDS,
        }
    ...
    def normalize(self, text: str) -> tuple[str, dict[str, Any]]:
        ...
        text = PatternNormalizer.normalize_whitespace(text)
        if text != original:
            transformations.append("whitespace_normalized")

        # 2. Fix common typos
        text = PatternNormalizer.fix_common_typos(text)
        if text != original:
        ...
        # 5. Normalize case
        case_normalized = PatternNormalizer.normalize_case(text)
