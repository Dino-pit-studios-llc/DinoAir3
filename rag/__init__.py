...
from utils.safe_imports import SafeImportError, safe_import_from_map
...
_SECURE_TEXT_EXTRACTOR_MODULE = "rag.secure_text_extractor"
...
_EXPORTS: dict[str, tuple[str, str]] = {
    ...
    "SecureTextExtractor": (_SECURE_TEXT_EXTRACTOR_MODULE, "SecureTextExtractor"),
    "create_secure_text_extractor": (
        _SECURE_TEXT_EXTRACTOR_MODULE,
        "create_secure_text_extractor",
    ),
    "extract_text_secure": (_SECURE_TEXT_EXTRACTOR_MODULE, "extract_text_secure"),
}
...
def __getattr__(name: str) -> Any:  # PEP 562 lazy loader
    try:
        if name == "DefaultContextProvider":
            # Map to EnhancedContextProvider using allowlisted resolution
            return safe_import_from_map(
                {
        ...
        # Resolve from the static export map via safe helper
        return safe_import_from_map(_EXPORTS, name)
    except SafeImportError as e:
        # Preserve Python module attribute semantics
