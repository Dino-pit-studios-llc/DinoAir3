...
class FileChunker:
    ...
    def _calculate_overlaps(
        self, chunk_index: int, current_pos: int, chunk_end: int, text_length: int
    ) -> tuple[int, int]:
        """Calculate overlap values for previous and next chunks.
        ...
        Returns:
            Tuple of (overlap_prev, overlap_next)
        """
        overlap_prev = 0
        overlap_next = 0
    ...
    def _get_next_position(self, chunk_end: int, chunks: list[TextChunk]) -> int:
        """Determine next position ensuring progress is made.
        ...
        if chunks and next_pos <= chunks[-1].metadata.start_pos:
            next_pos = chunk_end
    ...
    def chunk_by_paragraphs(self, text: str) -> list[TextChunk]:
        ...
        paragraphs = FileChunker._split_paragraphs(text)
    ...
    def _find_code_boundaries(
        ...
        for match in self.code_block_patterns["function"].finditer(code):
            ...
            end = FileChunker._find_code_block_end(code, start, language)
            if end > start:
    ...
    @staticmethod
    def _find_code_block_end(code: str, start: int, language: str) -> int:
        """
        ...
        for i, line in enumerate(lines):
            ...
            in_string, string_char, brace_count = FileChunker._update_string_brace(
                line, in_string, string_char, brace_count, base_indent
            )

            if FileChunker._is_block_end(
                i,
                in_string,
                string_char,
                brace_count,
