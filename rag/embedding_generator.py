...
class EmbeddingGenerator:
    ...
    def generate_embeddings_batch(
        ...
        try:
            valid_texts, valid_indices = self._prepare_valid_texts(texts)
    ...
    def _prepare_valid_texts(self, texts: list[str]) -> tuple[list[str], list[int]]:
        """Prepare valid texts by filtering and truncating.

        Args:
            texts: List of input texts

        Returns:
            Tuple of (valid_texts, valid_indices)
        """
        valid_texts = []
        valid_indices = []
    ...
    def _create_zero_embeddings(self, count: int) -> list[np.ndarray]:
        ...
        return [np.zeros(dim, dtype=np.float32) for _ in range(count)]

    def _determine_batch_size(self, batch_size: int | None) -> int:
        """Determine appropriate batch size.

        Args:
            batch_size: User-provided batch size or None
    ...
    def _generate_embeddings(
        ...
        Args:
            valid_texts: List of texts to embed
    ...
    def _reorder_embeddings(
        ...
        Args:
            ...
            total_count: Total number of original texts
