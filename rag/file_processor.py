...
class FileProcessor:
    ...
    @staticmethod
    def process_file(file_path: str, **kwargs) -> dict[str, Any]:  # noqa: D401
        """
    ...
    def get_processing_stats() -> dict[str, Any]:
        ...
        return FileProcessor.get_performance_stats()

    @staticmethod
    def clear_caches() -> None:
        """No-op in base processor; subclasses may override."""
        # No caches to clear in base class; subclasses may override.
        return
    ...
    def _find_files(self, root: str, recursive: bool, file_extensions: Iterable[str]) -> list[str]:
        ...
        """
        try:
            normalized_exts = self._normalize_extensions(file_extensions)
    ...
    @staticmethod
    def _normalize_extensions(file_extensions: Iterable[str]) -> set[str]:
    ...
    def _find_files_non_recursive(self, root: str, normalized_exts: set[str]) -> list[str]:
        ...
        Args:
            root: Root directory to scan
            normalized_exts: Set of normalized file extensions
        ...
        try:
            for name in os.listdir(root):
                ...
                if self._should_include_file(path, name, normalized_exts):
                    files.append(os.path.normpath(path))
        except FileNotFoundError:
            return []
        return files

    def _find_files_recursive(self, root: str, normalized_exts: set[str]) -> list[str]:
        ...
        for dirpath, _dirnames, filenames in os.walk(root):
            for name in filenames:
                if self._matches_extension(name, normalized_exts):
    ...
    @staticmethod
    def _should_include_file(path: str, name: str, normalized_exts: set[str]) -> bool:
    ...
    def _matches_extension(name: str, normalized_exts: set[str]) -> bool:
        """Check if filename matches any of the allowed extensions.
        ...
        Returns:
            True if no extensions specified or file matches an extension
        """
        if not normalized_exts:
            return True
        return os.path.splitext(name)[1].lower() in normalized_exts

    @staticmethod
    def _calculate_file_hash(file_path: str, chunk_size: int = 1024 * 1024) -> str:
        """
        ...
        allowed_filenames = {"allowed1.txt", "allowed2.txt"}
        filename = os.path.basename(file_path)
        if filename not in allowed_filenames:
            raise ValueError(f"Unallowed file: {filename}")
        # Construct safe path
        safe_path = os.path.join("/safe/base/dir", filename)
