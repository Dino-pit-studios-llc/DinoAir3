...
class OptimizedFileProcessor(FileProcessor):
    ...
    @staticmethod
    def _gather_file_stats(file_path: str) -> tuple[int, datetime, str]:
    ...
    def _should_skip(
        ...
        if existing and not force_reprocess:
            try:
                ...
                OptimizedFileProcessor._should_skip_error_count += 1
                _cnt = OptimizedFileProcessor._should_skip_error_count
                if _cnt == 1 or _cnt % 100 == 0:
                    _DBG_LOG.debug("skip-check failed: %s", e)
        return None

    def _compute_chunk_params(self) -> tuple[int, int]:
        cs = max(100, int(self.chunk_size or 1000))
        ov = max(0, min(int(self.chunk_overlap or 200), cs - 1))
        return cs, ov
    ...
    def _chunk_text(text: str, chunk_size: int, overlap: int) -> list[dict[str, Any]]:
        ...
        while start < n:
            ...
            start = end - overlap if overlap and end < n else end
            idx += 1
        return chunks
    ...
    def process_file(self, file_path: str, **kwargs) -> dict[str, Any]:
        ...
        validation_result = self._validate_and_prepare_file(file_path, force_reprocess)
        ...
        chunk_ids = self._store_chunks(chunks, file_id, file_type, file_path)

        # Generate embeddings if enabled
        try:
            embeddings_generated = self._process_embeddings(chunks, chunk_ids)
    ...
    def _validate_and_prepare_file(self, file_path: str, force_reprocess: bool) -> dict[str, Any]:
        ...
        Returns:
            Dict with file_id, file_type, text or error
        """
        ...
        text, read_error = self._safe_read_file(file_path)
        if read_error:
            return read_error
        ...
        return {"file_id": file_id, "file_type": file_type, "text": text}
    ...
    def _create_chunks(self, text: str) -> list[dict[str, Any]]:
        ...
        """
        cs, ov = self._compute_chunk_params()
        return OptimizedFileProcessor._chunk_text(text, cs, ov)
    ...
    def _process_embeddings(self, chunks: list[dict[str, Any]], chunk_ids: list[str]) -> int:
        ...
        """
        if not self.generate_embeddings or not chunk_ids:
            return 0
        ...
        return self._generate_and_store_embeddings(
            chunk_ids=chunk_ids,
    ...
    def _partition_embeddings(
        self, chunk_ids: list[str], chunk_texts: list[str]
    ...
    def _generate_batches(
        ...
        if embeddings_to_generate:
            ...
            for start in range(0, len(embeddings_to_generate), batch_size):
                ...
                for (i, chunk_id, _), embedding in zip(batch, results, strict=False):
    ...
    def _store_embeddings(
        ...
        count = 0
        for _, chunk_id, embedding in new_embeddings + cached_embeddings:
            if self.enable_caching:
