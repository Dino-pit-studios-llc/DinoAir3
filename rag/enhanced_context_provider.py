...
class SearchHistory:
    ...
    def get_suggestions(self, partial_query: str, limit: int = 5) -> list[str]:
        ...
        suggestions = self._get_recent_matching_queries(partial_lower, limit)
    ...
    def _get_recent_matching_queries(self, partial_lower: str, limit: int) -> list[str]:
        """Get recent queries that start with the partial query.

        Args:
            partial_lower: Lowercase partial query string
            limit: Maximum number of suggestions to return

        Returns:
            List of matching query strings
        """
        suggestions: list[str] = []
    ...
    def _add_popular_matching_terms(self, partial_lower: str, suggestions: list[str], limit: int) -> list[str]:
        ...
        Args:
            partial_lower: Lowercase partial query string
            suggestions: Existing suggestions list to add to
            limit: Maximum total suggestions desired
...
class EnhancedContextProvider:
    ...
    def _build_context_item(self, result: Any) -> dict[str, Any]:
        ...
        context_item["preview"] = EnhancedContextProvider._create_preview(
    ...
    @staticmethod
    def _create_preview(content: str, highlight_terms: list[str] = None) -> str:
    ...
    def export_results(
        ...
        try:
            ...
            if format == "json":
                EnhancedContextProvider._export_json(results, file_path)
            elif format == "csv":
                EnhancedContextProvider._export_csv(results, file_path)
    ...
    @staticmethod
    def _export_json(results: list[dict[str, Any]], file_path: str) -> None:
        """Export results as JSON"""
        allowed_filenames = {"export.json", "results.json", "data_export.json"}
    ...
    @staticmethod
    def _export_csv(results: list[dict[str, Any]], file_path: str) -> None:
        ...
        filename = os.path.basename(file_path)
        allowed_filenames = ["results.csv", "export.csv"]
        if filename not in allowed_filenames:
            raise ValueError(f"Export filename '{filename}' is not permitted.")
