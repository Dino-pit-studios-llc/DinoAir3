...
class ModelManager(ShutdownMixin):
    """
    ...
    def _load_model(self, name: str, model_path: Path | None = None) -> BaseModel:
        ...
        ModelManager._check_memory_availability(name)
    ...
    @staticmethod
    def _check_memory_availability(model_name: str) -> None:
        """
        ...
        if available_gb < capabilities.recommended_memory_gb:
            logger.warning(
                "Memory below recommended for model '%s'. Available: %.1fGB, Recommended: %sGB",
                model_name,
                available_gb,
                capabilities.recommended_memory_gb,
            )
    ...
    def get_memory_usage(self) -> dict[str, Any]:
        """Get memory usage information. Returns: Memory usage statistics"""
        memory = psutil.virtual_memory()
    ...
    def switch_default_model(self, name: str) -> None:
        """Switch the default model. Args: name: New default model name"""
        if not model_exists(name):
            raise KeyError(f"Model '{name}' not found in registry")
    ...
    def _cleanup_resources(self) -> None:
        """Cleanup all model resources during shutdown"""
