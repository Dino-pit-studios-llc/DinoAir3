...
class ModelDownloader:
    ...
    def download_model(
        ...
        filename = ModelDownloader._get_filename_from_url(url) or f"{model_name}.gguf"
    ...
    def _verify_checksum(self, file_path: Path, expected_checksum: str) -> bool:
        ...
        """
        allowed_filenames = {"example1.bin", "example2.bin"}
        filename = file_path.name
        if filename not in allowed_filenames:
            raise ValueError(f"Invalid file name: {filename}")
        safe_path = file_path.parent / filename
    ...
    @staticmethod
    def estimate_download_time(file_size: int, bandwidth_mbps: float = 10.0) -> float:
        """
