...
class ParserModule:
    ...
    def parse(self, input_text: str) -> list[CodeBlock]:
        ...
        try:
            # Split into blocks
            raw_blocks = ParserModule._identify_blocks(input_text)
    ...
    def _identify_blocks(text: str) -> list[str]:
        ...
        # Split text into blocks using blank lines as delimiters
        blocks = []
        current_block = []
        ...
        # If no blocks were created (all text was one continuous block),
    ...
    def _process_line_metadata(
        ...
        indent_chars: set[str],
        current_max_indent: int,
    ) -> int:
        """
    ...
    def _calculate_python_score(self, line: str) -> float:
        ...
        if ParserModule._is_valid_python(line):
            score += 0.2
    ...
    def _is_ast_transition(self, prev_line: str, curr_line: str) -> bool:
        ...
        """
        prev_is_python = ParserModule._is_valid_python(prev_line.strip())
        curr_is_python = ParserModule._is_valid_python(curr_line.strip())
    ...
    def _is_valid_python(line: str) -> bool:
        ...
        if ParserModule._try_parse_as_statement(line):
            return True
        if ParserModule._try_parse_as_expression(line):
            return True
        if ParserModule._try_parse_with_colon(line):
            return True
        if ParserModule._matches_incomplete_pattern(line):
            return True

        return False

    @staticmethod
    def _try_parse_as_statement(line: str) -> bool:
        """Try to parse line as a Python statement"""
        try:
            ast.parse(line)
            return True
        except SyntaxError:
    ...
    @staticmethod
    def _try_parse_as_expression(line: str) -> bool:
        """Try to parse line as a Python expression"""
        try:
    ...
    @staticmethod
    def _try_parse_with_colon(line: str) -> bool:
        """Try to parse line with added colon for control structures"""
        if line.endswith(":"):
            return False
        try:
            ast.parse(line + ":")
    ...
    @staticmethod
    def _matches_incomplete_pattern(line: str) -> bool:
        """Check if line matches common incomplete code patterns"""
        incomplete_patterns = [
    ...
    def _parse_chunk(self, chunk_text: str, start_line: int) -> list[CodeBlock]:
        ...
        raw_blocks = ParserModule._identify_blocks(chunk_text)
        code_blocks = []
