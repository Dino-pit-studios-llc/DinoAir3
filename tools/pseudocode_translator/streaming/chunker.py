...
class CodeChunker:
    ...
    @staticmethod
    def _find_ast_boundaries(tree: ast.AST, lines: list[str]) -> list[dict[str, Any]]:
        """
        ...
        for node in body:
            boundary = CodeChunker._create_boundary_from_node(node, lines)
            boundaries.append(boundary)

        return boundaries

    @staticmethod
    def _create_boundary_from_node(node: ast.AST, lines: list[str]) -> dict[str, Any]:
    ...
    @staticmethod
    def _create_definition_boundary(node: ast.AST, lines: list[str]) -> dict[str, Any]:
        """Create boundary for function, class, or import definition"""
        start_line = node.lineno - 1
        end_line = node.end_lineno or node.lineno

        # Include decorators if present
        start_line = CodeChunker._adjust_for_decorators(node, start_line)

        # Include any preceding comments or docstrings
        start_line = CodeChunker._find_block_start(lines, start_line)
    ...
    @staticmethod
    def _adjust_for_decorators(node: ast.AST, start_line: int) -> int:
    ...
    @staticmethod
    def _create_docstring_boundary(node: ast.AST, lines: list[str]) -> dict[str, Any]:
        """Create boundary for module-level docstring"""
        return {
            "type": "ModuleDocstring",
            "name": None,
            "start_line": node.lineno - 1,
            "end_line": node.end_lineno or node.lineno,
            "size": CodeChunker._get_node_size("".join(lines), node),
            "node": node,
        }

    @staticmethod
    def _create_toplevel_boundary(node: ast.AST, lines: list[str]) -> dict[str, Any]:
        """Create boundary for other top-level code"""
        return {
            "type": "TopLevelCode",
            "name": None,
            "start_line": node.lineno - 1,
    ...
    @staticmethod
    def _find_block_start(lines: list[str], start_line: int) -> int:
        """
    ...
    def _create_chunk_from_boundaries(
        boundaries: list[dict[str, Any]], lines: list[str], chunk_index: int
    ) -> CodeChunk:
        """
    ...
    @staticmethod
    def _find_line_split_point(lines: list[str]) -> int:
        """
    ...
    @staticmethod
    def _get_node_size(source: str, node: ast.AST) -> int:
