...
class BufferEntry:
    ...
    def get_data(self, decompress: bool = True) -> Any:
        ...
        if self.compressed and decompress:
            return BufferEntry._decompress(self.data)
        return self.data
...
class StreamBuffer:
    ...
    def _evict_entry(self) -> bool:
        ...
        logger.debug("Evicted chunk %s, freed %s bytes", key_to_evict, entry.size)
        return True
    ...
    def persist_to_disk(self, path: str | None = None):
        ...
        try:
            with self._lock:
                ...
                logger.info("Persisted %s chunks to %s", len(save_data), save_path)

        except Exception as e:
            logger.error("Error persisting buffer: %s", e)
    ...
    def load_from_disk(self, path: str | None = None) -> bool:
        ...
        try:
            ...
            with self._lock:
                ...
                logger.info("Loaded %s chunks from %s", len(save_data), load_path)
                return True

        except Exception as e:
            logger.error("Error loading buffer: %s", e)
            return False
