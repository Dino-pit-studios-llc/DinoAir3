...
class TranslatorError(Exception):
    ...
    def format_error(
        ...
        if include_context:
            ...
            if code_context:
                parts.append("\n  Code context:")
                parts.append(TranslatorError._indent(code_context, 4))
...
class ValidationError(TranslatorError):
    ...
    def _add_validation_suggestions(self, code: str, message: str, val_type: str):
        ...
        if val_type == "syntax":
            self._add_syntax_suggestions()
        elif val_type == "logic":
            self._add_logic_suggestions(message)
        elif val_type == "security":
            self._add_security_suggestions()

    def _add_syntax_suggestions(self):
    ...
    def _add_logic_suggestions(self, message: str):
        """Add suggestions for logic errors"""
        if "undefined" in message.lower():
            self._add_undefined_variable_suggestions(message)
        if "return" in message.lower():
            self._add_return_suggestions()

    def _add_undefined_variable_suggestions(self, message: str):
        """Add suggestions for undefined variable errors"""
        import re
        ...
        self._add_typo_suggestions(var_name)

    def _add_typo_suggestions(self, var_name: str):
        ...
        for common in common_vars:
            if ValidationError._is_similar(var_name, common):
                self.add_suggestion(f"Did you mean '{common}' instead of '{var_name}'?")

    def _add_return_suggestions(self):
        """Add suggestions for return statement errors"""
        self.add_suggestion("Add a return statement to the function")
        self.add_suggestion("Check that all code paths return a value")

    def _add_security_suggestions(self):
        """Add suggestions for security errors"""
