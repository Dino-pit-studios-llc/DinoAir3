...
class ParallelProcessor:
    ...
    def __init__(
        ...
        logger.info(
            "Initialized ParallelProcessor with mode=%s, max_workers=%s",
            mode.value,
            self.max_workers,
        )
    ...
    def process_files(
        ...
        for _i, file_path in enumerate(file_paths):
            ...
            try:
                ...
                logger.error("Error accessing file %s: %s", path, e)
    ...
    def _process_single_file(self, task: FileTask) -> ProcessingResult:
        ...
        try:
            ...
            logger.error("Failed to process %s: %s", task.file_path, e)
...
class BatchProcessor:
    ...
    def process_batch(
        ...
        logger.info("Processing batch of %s files", len(all_files))
