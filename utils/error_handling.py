...
"""Module for retrying operations on failure with configurable settings.

This module provides a decorator to retry operations on failure based on the provided
...
def _execute_with_retry(
    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            last_exception = e
            _handle_retry_failure(config, func, attempt, e)
...
def _handle_retry_failure(config: RetryConfig, func: Callable[..., Any], attempt: int, error: Exception) -> None:
    ...
    if attempt < config.max_attempts - 1:
        _log_retry_attempt(config, func, attempt, error)
...
def circuit_breaker(
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        """Creates a decorator that applies circuit breaker logic to the target function."""
        breaker_name = name or func.__name__

        @functools.wraps(func)
        def wrapper(*args: Any, **kwargs: Any) -> T:
            """Wrapper that manages the circuit breaker state and executes the function."""
...
def with_timeout(
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        """Decorator function that wraps the target function with a timeout context.
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            Returns:
                The result of the decorated function if completed before timeout.
            Raises:
...
def handle_error(error: Exception, context: ErrorContext | None = None) -> dict[str, Any]:
    ...
    Returns a dict with:
      - success: False
      - error: the exception object (tests call str(...) on this)
      - context: basic context info when provided
    """
    ...
    # Log in a best-effort manner
    try:
        if enhanced_logging_available:
            logger.error(
                "Handled error in operation '%s': %s",
                context.operation if context and context.operation else "unknown",
                error,
                extra={
            ...
            logger.error("Handled error: %s", error)
...
def _log_safe_failure(message: str, op_name: str, e: Exception, *, async_operation: bool) -> None:
    ...
    if enhanced_logging_available:
        ...
        if async_operation:
            extra["async_operation"] = True
        logger.error(message, e, extra=extra)
    else:
