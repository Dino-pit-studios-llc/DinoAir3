...
# Redaction string constant
_REDACTED_VALUE = "***REDACTED***"

# Collect env-based secrets to redact (values)
_ENV_SECRET_VALUES = {
    v for k, v in os.environ.items() if k.upper().startswith("DINOAIR_") and v
}
...
class RedactionFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        This filter always returns True to allow all log records through,
        as its purpose is to modify (redact) sensitive data, not to block logs.

        Args:
            record: The log record to process and potentially redact

        Returns:
            Always True - this filter modifies records but never blocks them
        """
        try:
            ...
            for attr in ("msg",):
                ...
                if isinstance(val, dict):
                    setattr(
                        record, attr, self._redact_mapping(cast("dict[str, Any]", val))
                    )
            ...
            # Ensure filter does not block logging on error
            return True
        return True
