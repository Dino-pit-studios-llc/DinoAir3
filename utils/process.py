...
class SecurityConfig:
    ...
    def get_redact_env_keys(self) -> list[str]:
        ...
        if self._config:
            try:
                return self._config.get(
                    "security.process.logging.redact_env_keys",
                    self._default_redact_env_keys,
                )
            except (KeyError, TypeError, AttributeError):
                logger.warning(
...
def _ensure_list_command(command: Sequence[str]) -> None:
    """Validate that command is a non-empty sequence of strings."""
    ...
    for arg in args:
        ...
        for pattern in patterns:
            try:
                ...
                logger.warning(
                    "Invalid regex pattern for binary validation (%r): %s",
                    REDACTED_TEXT,
                    e,
                )
...
def _log_invocation(
    ...
    try:
        ...
        log_data = {
            "binary": REDACTED_TEXT,
            "args_count": len(command) - 1,
            "command": redacted_command,
            "cwd": REDACTED_TEXT if cwd else None,
        ...
        with contextlib.suppress(OSError, ValueError, TypeError):
            # Silently ignore logging exceptions
...
def _preflight_process(
    ...
    """
    Shared validation and logging for process execution functions.
    Returns the normalized binary name after validation.
    """
...
def safe_run(
    ...
    """
    binary = _preflight_process(command, allowed_binaries, cwd, env)
    ...
    try:
        ...
        logger.error("Binary not found: %s", REDACTED_TEXT)
        raise
    except PermissionError:
        logger.error("Permission denied executing: %s", REDACTED_TEXT)
        raise
...
def safe_popen(
    ...
    Raises:
        ...
        PermissionError: If binary not in effective allowlist.
    """
    _preflight_process(command, allowed_binaries, cwd, env)
...
def run_subprocess(
    ...
    """
    try:
        # Permit the invoked binary by default for tests
        ...
        logger.error("run_subprocess failed: %s", e)
        return {"success": False, "stdout": "", "stderr": str(e), "returncode": -1}


def kill_process(proc: Any) -> bool:
    """
    Attempt to terminate a subprocess.Popen-like handle safely.
    Returns True if a terminate/kill was attempted without raising.
    """
    try:
        # Support either Popen object or PID
        import os
        import signal
        ...
        if hasattr(proc, "terminate"):
            try:
                ...
                # First attempt failed, try kill as fallback
                logger.debug("Process terminate failed (%s), attempting kill", e)
                try:
                    proc.kill()
                except OSError as kill_error:
                    logger.debug("Process kill also failed: %s", kill_error)
            return True
        # Fallback: numeric PID
        if isinstance(proc, int):
            try:
                ...
                logger.debug("Failed to kill process %s: %s", proc, e)
            return True
    except Exception as e:
        logger.debug("Unexpected error in kill_process: %s", e)
    return False
