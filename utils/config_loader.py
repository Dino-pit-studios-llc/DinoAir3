...
class ConfigLoader:
    ...
    def __init__(self, config_path: Path | dict[str, Any] | None = None):
        """
        Construct a ConfigLoader.

        Supports either:
         - Path to a JSON config file (default behavior)
         - dict with config data (used by tests)
        """
        ...
        if config_path is not None:
            # config_path is a Path in this branch
            self.env_path = Path(config_path).parent / ".env"
        else:
            self.env_path = Path(__file__).parent.parent.parent / ".env"
    ...
    def _apply_env_overrides(self) -> None:
        ...
        for env_key, config_key in env_mappings.items():
            self._apply_env_override(env_key, config_key)

    @staticmethod
    def _get_env_mappings() -> dict[str, str]:
        """Get environment variable to config key mappings.

        Returns:
            Dictionary mapping environment variables to config keys
        """
        return {
            "DEBUG": "app.debug",
    ...
    def _apply_env_override(self, env_key: str, config_key: str) -> None:
        ...
        if env_key in self.env_vars or env_key in os.environ:
            ...
            value = self._convert_env_value(raw_value)
            self.set(config_key, value, save=False)

    @staticmethod
    def _convert_env_value(raw_value: str) -> Any:
        ...
        try:
            ...
            try:
                ...
                return raw_value  # keep as string
    ...
    def save_config(self) -> None:
        ...
        save_json_config(self.config_data, self.config_path)

    async def save_config_async(self) -> None:
        """Save configuration to file asynchronously"""
        await save_json_config_async(self.config_data, self.config_path)
        try:
...
def save_json_config(config_data: Any, config_path: Path, indent: int = 4) -> bool:
    """
    Centralized utility for saving configuration data to JSON files.

    Args:
        config_data: Data to save (dict, dataclass, or serializable object)
    ...
    try:
        ...
        if hasattr(config_data, "__dict__"):
            ...
            data_to_save = config_data.__dict__
        elif hasattr(config_data, "_asdict"):
            # Handle namedtuples
            data_to_save = config_data._asdict()
        else:
            # Assume it's already serializable (dict, etc.)
...
async def save_json_config_async(
    ...
    try:
        ...
        async with aiofiles.open(config_path, "w", encoding="utf-8") as f:
            await f.write(json.dumps(data_to_save, indent=indent, default=str))
        return True
    except (OSError, TypeError, ValueError, AttributeError) as e:
        Logger().error(f"Error saving config to {config_path}: {e}")
        return False
