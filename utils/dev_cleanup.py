...
class UserDataCleanupManager:
    ...
    def get_user_data_directories() -> dict[str, Path]:
        ...
        try:
            ...
            LOGGER.warning("Could not determine default user data directory: %s", e)
    ...
    @staticmethod
    def analyze_user_data() -> dict[str, dict]:
        """Analyze user data directories and their contents.

        Returns:
            Dict mapping location to analysis results
        """
        directories = UserDataCleanupManager.get_user_data_directories()
        analysis = {}
        ...
        for location, path in directories.items():
            try:
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    for file in files:
                        ...
                        try:
                            ...
                            LOGGER.warning("Could not access %s", file_path)
                ...
                LOGGER.error("Error analyzing %s: %s", path, e)
            ...
            for test_dir in temp_dir.glob(pattern):
                ...
                try:
                    ...
                    if os.path.getmtime(test_dir) > cutoff_time:
                        if self.verbose:
                            LOGGER.debug("Skipping recent test directory: %s", test_dir)
                        continue

                    # Calculate size before removal
                    size_mb = UserDataCleanupManager._calculate_directory_size(test_dir)

                    if self.dry_run:
                        LOGGER.info(
                            "Would remove test directory: %s (%.2f MB)",
                            test_dir,
                            size_mb,
                        )
                    else:
                    ...
                    LOGGER.warning("Could not remove %s: %s", test_dir, e)
    ...
    def cleanup_repository_user_data(self, backup: bool = True) -> dict[str, int]:
        ...
        # Calculate size
        size_mb = UserDataCleanupManager._calculate_directory_size(repo_user_data)
        ...
        if self.dry_run:
            LOGGER.info(
                "Would remove repository user_data: %s (%.2f MB)",
                repo_user_data,
                size_mb,
            )
        else:
            try:
                LOGGER.info(
                    "Removing repository user_data: %s (%.2f MB)",
                    repo_user_data,
                    size_mb,
                )
                ...
                LOGGER.error("Could not remove %s: %s", repo_user_data, e)
    ...
    def cleanup_database_connections(
        ...
        try:
            ...
            LOGGER.error("Error cleaning database connections: %s", e)
    ...
    def full_cleanup(
        ...
        LOGGER.info(
            "Cleanup completed. Space freed: %.2f MB, Items removed: %d",
            total_space_freed,
            total_items_removed,
        )
...
def main():
    ...
    parser.add_argument(
        "--analyze",
        action="store_true",
        help="Analyze user data directories without cleaning",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--no-repo-cleanup",
        action="store_true",
        help="Skip cleaning repository user_data",
    )
    parser.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip creating backup of repository data",
    )
    ...
    if args.analyze:
        cleanup_manager.analyze_user_data()
    else:
