...
class WatchdogConfigValidator:
    ...
    def create_config_summary(config: dict[str, Any]) -> str:
        ...
        categories = WatchdogConfigValidator._get_config_categories()

        for category, params in categories.items():
            WatchdogConfigValidator._add_category_lines(lines, category, params, config)

        return "\n".join(lines)

    @staticmethod
    def _get_config_categories() -> dict[str, list[str]]:
        """Get configuration parameter categories.

        Returns:
            Dictionary mapping category names to parameter lists
        """
        return {
    ...
    def _add_category_lines(lines: list[str], category: str, params: list[str], config: dict[str, Any]) -> None:
        ...
        Args:
            ...
            params: List of parameter names in this category
            config: Configuration dictionary
        """
        lines.append(f"\n{category}:")
        for param in params:
            if param in config:
                WatchdogConfigValidator._add_param_lines(lines, param, config[param])

    @staticmethod
    def _add_param_lines(lines: list[str], param: str, value: Any) -> None:
        """Add formatted lines for a configuration parameter.
