...
class DependencyContainer:  # pylint: disable=too-many-instance-attributes
    ...
    # ------------------------------------------------------------------
    # Backward-compatible convenience API expected by tests:
    # container.register(name, instance_or_factory)
    # If a non-callable instance is provided, treat as singleton instance.
    # If a callable is provided, register as singleton factory.
    # ------------------------------------------------------------------
    def register(self, name: str, obj: Any) -> "DependencyContainer":
        """Register a dependency using a simplified API.

        - If obj is a unittest.mock.Mock instance, treat it as an instance (not a factory)
        - Else if obj is callable: register as singleton with factory
        - Else: register the provided instance as a singleton
        """
        # Detect unittest.mock.Mock without importing at module top
        is_mock = False
        try:
            from unittest.mock import Mock as _Mock

            is_mock = isinstance(obj, _Mock)
        except Exception:
            is_mock = False

        if is_mock:
            return self.register_instance(name, instance=obj, dependencies=[])

        if callable(obj):
            # Register as singleton with factory (no dependencies metadata)
            # Avoid calling obj() here (could have side effects), infer type lazily via factory
    ...
    def _match_params_by_name(sig: inspect.Signature, resolved_dependencies: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            ...
            resolved_dependencies: Available dependencies

        Returns:
            Dictionary of matched parameters
        """
        return {
            param_name: resolved_dependencies[param_name]
            for param_name in sig.parameters
    ...
    @staticmethod
    def _match_params_by_type(
        sig: inspect.Signature,
        resolved_dependencies: dict[str, Any],
        already_matched: dict[str, Any],
    ) -> dict[str, Any]:
        """Match parameters by type annotation.

        Args:
            sig: Function signature
            resolved_dependencies: Available dependencies
            already_matched: Parameters already matched by name

        Returns:
            Dictionary of newly matched parameters
        """
        kwargs: dict[str, Any] = {}
    ...
    def _find_instance_by_type(annotation: Any, resolved_dependencies: dict[str, Any]) -> Any | None:
        ...
        Args:
            annotation: Type annotation to match
            resolved_dependencies: Available dependencies

        Returns:
            Matching instance or None
        """
        for dep_instance in resolved_dependencies.values():
            try:
