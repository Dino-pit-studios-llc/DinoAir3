...
try:
    ...
    try:
        ...
        class _DummyQTimer:
            ...
            @staticmethod
            def set_single_shot(*_args: Any, **_kwargs: Any) -> None:
                """No-op stub for dummy QTimer; intentionally does nothing."""
...
# -----------------------------------------------------------------------------
# Lightweight decorators expected by tests/test_utils_core.py
...
def debounce(wait_seconds: float):
    ...
    Executes the wrapped function immediately, then suppresses subsequent
    calls made within 'wait_seconds'.
    """
...
def throttle(wait_seconds: float):
    ...
    Executes the wrapped function at most once every 'wait_seconds'.
    """
    from functools import wraps
