...
class EnhancedLogFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if not self._passes_pattern_filters(record):
            return False

        return bool(self._passes_sampling(record))

    def _passes_level_filter(self, record: logging.LogRecord) -> bool:
        """Check if record passes module level filter."""
        module_level = self.config.level_filters.get(record.name)
        if module_level is not None and record.levelno < module_level:
            return False
        return True

    def _passes_pattern_filters(self, record: logging.LogRecord) -> bool:
        """Check if record passes include/exclude pattern filters."""
    ...
    def _passes_sampling(self, record: logging.LogRecord) -> bool:
        """Check if record passes sampling rate filter."""
...
class EnhancedJsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        EnhancedJsonFormatter._add_extra_fields(record, log_entry)
    ...
    def _process_queue(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                self._handle_emit_error("async handler drop", e)
            except Exception as e:
                self._handle_emit_error("async handler unexpected error", e)

    def _handle_emit_error(self, error_type: str, error: Exception) -> None:
        """Handle errors during log record emission with rate limiting.
...
class EnhancedLogger:
    ...
    def __new__(cls, *args: Any, **kwargs: Any) -> "EnhancedLogger":
        # Accept arbitrary args for compatibility (e.g., name), but ignore for singleton creation
    ...
    def __init__(self, name: str | None = None):
        ...
        # Provide a simple .name attribute for compatibility with tests
        if name is not None:
            self.name = name
        elif not hasattr(self, "name"):
            self.name = "EnhancedLogger"
    ...
    def _setup_logging(self):
        ...
        file_handler = self._create_file_handler(log_dir, formatter)
        console_handler = self._create_console_handler(formatter)

        self._add_handlers_to_root([file_handler, console_handler])
    ...
    def _create_file_handler(self, log_dir: Path, formatter: logging.Formatter) -> logging.Handler:
        ...
        Returns:
            Configured file handler (possibly wrapped in async handler)
        """
    ...
    def _create_console_handler(self, formatter: logging.Formatter) -> logging.Handler:
        ...
        Returns:
            Configured console handler (possibly wrapped in async handler)
        """
    ...
    def _add_handlers_to_root(self, handlers: list[logging.Handler]) -> None:
