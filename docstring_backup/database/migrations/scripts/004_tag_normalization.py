...
class TagNormalizationMigration(BaseMigration):
    ...
    def up(self, conn: sqlite3.Connection) -> None:
        ...
        try:
            ...
            TagNormalizationMigration._normalize_existing_tags(cursor)

            # Step 2: Add expression indexes for efficient JSON tag queries
            TagNormalizationMigration._add_tag_indexes(cursor)

            # Step 3: Add constraints and validation
            TagNormalizationMigration._add_tag_validation(cursor)
    ...
    def _normalize_existing_tags(cursor: sqlite3.Cursor) -> None:
        ...
        for note_id, normalized_tags_json in notes_to_update:
            cursor.execute(
                "UPDATE note_list SET tags = ? WHERE id = ?",
                (normalized_tags_json, note_id),
            )
    ...
    @staticmethod
    def down(conn: sqlite3.Connection, some_other_param) -> None:
