...
class FileSearchDB:
    ...
    def __init__(self, user_name: Any | None = None):
        """
        Initialize FileSearchDB with user-specific database connection or an existing manager.

        Args:
            user_name: Either a username for user-specific database OR a DatabaseManager instance.
        """
        self.logger = Logger()

        # Allow passing a DatabaseManager directly for tests/backwards-compat
        if isinstance(user_name, DatabaseManager):
            self.db_manager = user_name
            # Try to extract a name if present, otherwise use default
            self.user_name = getattr(user_name, "user_name", "default_user")
        else:
            self.db_manager = DatabaseManager(user_name)
            self.user_name = user_name or "default_user"

        # Back-compat attribute expected by tests
        self._db_manager = self.db_manager
    ...
    def add_indexed_file(
        ...
        *,
        # Back-compat/alternate signature used in tests
        file_size: int | None = None,
        content_hash: str | None = None,  # accepted but not persisted separately
    ) -> dict[str, Any]:
        """
        Add a new file to the index.

        Supports both the current signature and a legacy/alternate test signature:
          - size or file_size (bytes)
          - modified_date optional (defaults to now)

        Args:
            file_path: Path to the file
            file_hash: Hash of the file content
            size: File size in bytes (preferred)
            modified_date: Last modification date of the file
            file_type: Type of the file (e.g., 'pdf', 'txt', 'docx')
            metadata: Additional metadata as dictionary
            file_size: Alternative parameter name for size (back-compat)
            content_hash: Ignored (present for back-compat only)

        Returns:
            Dict with success status and file_id or error message
        """
        try:
            # Retry on transient SQLite lock errors to improve concurrent write robustness
            max_attempts = 6
            backoff_base = 0.02  # seconds
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    # Handle 'database is locked' or 'table is locked' transient errors with backoff
            ...
            return {
                "success": False,
                "error": "Failed to index file due to database lock contention",
            }
        except Exception as e:
            ...
            with self._get_connection() as conn:
                ...
                if row:
                    file_info = {
                        ...
                        "file_hash": row[2],
                        "size": row[3],
                        "file_size": row[3],  # alias for tests expecting 'file_size'
                        "modified_date": row[4],
                        "indexed_date": row[5],
                        "file_type": row[6],
                        "status": row[7],
    ...
    def update_search_settings(self, setting_name: str, setting_value: Any) -> dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                # Generate unique ID
                setting_id = FileSearchDB._generate_id(setting_name)
    ...
    def __del__(self):
        """Ensure all DB connections are released promptly (important on Windows)."""
        try:
            if hasattr(self, "db_manager") and self.db_manager:
                # Close any tracked connections to release file locks
                cleanup = getattr(self.db_manager, "_cleanup_connections", None)
                if callable(cleanup):
                    cleanup()
        except Exception as e:
            # Never raise from destructor; log at debug level to avoid noisy teardown logs
            try:
