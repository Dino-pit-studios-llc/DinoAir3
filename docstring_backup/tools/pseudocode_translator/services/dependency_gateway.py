...
class DependencyAnalysisGateway:
    ...
    def analyze_and_annotate(self, blocks: list[Any]) -> None:
        ...
        resolver = DependencyAnalysisGateway._get_resolver()
        if resolver is not None:
            analyses = DependencyAnalysisGateway._safe_analyze_blocks(resolver, blocks)
    ...
    @staticmethod
    def _is_python_block(block) -> bool:
        block_type = getattr(block, "type", None)
        return str(getattr(block_type, "value", block_type)).lower() == "python"

    @staticmethod
    def _reset_block_metadata(block) -> None:
        try:
            block.metadata["defined_names"] = []
            block.metadata["required_imports"] = []
