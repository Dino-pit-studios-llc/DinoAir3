...
DEFAULT_BOOLEAN_FALSE = 0
DEFAULT_BOOLEAN_TRUE = 1
DEFAULT_VERSION = 1
DEFAULT_SIZE_BYTES = 0
DEFAULT_ARTIFACT_COUNT = 0
DEFAULT_WATCHDOG_RETENTION_DAYS = 7
DEFAULT_MAX_BACKUP_AGE_DAYS = 30
MAX_RECENT_NOTES_LIMIT = 100
BYTES_TO_MB_DIVISOR = 1024 * 1024
HOURS_PER_DAY = 24
SECONDS_PER_HOUR = 3600
MONTH_FORMAT_DIGITS = 2

# Path constants for duplicated literals
DEFAULT_WINDOWS_APPDATA = "~/AppData/Local"
DEFAULT_UNIX_XDG = "~/.local/share"
...
def _get_windows_data_directory() -> Path:
    """Get Windows user data directory with validation.
    ...
    if not is_safe_user_root(local_root_dir):
        local_root_dir = get_default_root_dir()

    return local_root_dir


def _get_unix_data_directory() -> Path:
    """Get Unix-like system data directory with XDG validation.

    Returns:
        Path: Validated Unix data directory
    """
    local_default_xdg = Path(DEFAULT_UNIX_XDG).expanduser().resolve()
    local_user_home = Path.home().resolve()

    xdg_data_home = os.environ.get("XDG_DATA_HOME")
    if not xdg_data_home:
        return local_default_xdg

    validated_path = _validate_xdg_data_home(xdg_data_home, local_user_home)
    return validated_path if validated_path else local_default_xdg


def _validate_xdg_data_home(xdg_path: str, user_home_path: Path) -> Path | None:
    """Validate XDG_DATA_HOME environment variable.

    Args:
        xdg_path: Value from XDG_DATA_HOME environment variable
        user_home_path: User's home directory path

    Returns:
        Validated path or None if validation fails
    """
    try:
        local_abs_env_path = _normalize_and_resolve_path(xdg_path)
        local_user_home_str = _get_user_home_with_sep(user_home_path)
...
def _normalize_and_resolve_path(path_str: str) -> Path:
    ...
    """
    local_norm_env_path = os.path.normpath(path_str)
    return Path(local_norm_env_path).resolve()


def _get_user_home_with_sep(home_dir: Path) -> str:
    """Get user home path string with trailing separator.

    Args:
        home_dir: User home directory path

    Returns:
        Path string with trailing separator
    """
    local_user_home_str = str(home_dir)
    if not local_user_home_str.endswith(os.sep):
        return local_user_home_str + os.sep
    return local_user_home_str


def _is_path_within_user_home(abs_path: Path, user_home_with_sep: str) -> bool:
    """Check if a path is within the user home directory.

    Args:
        abs_path: Absolute path to check
        user_home_with_sep: User home path with trailing separator

    Returns:
        True if path is within user home
    """
    return str(abs_path).startswith(user_home_with_sep)


def _get_or_create_user_data_path(
    local_root_dir: Path,
) -> Path:
    """Get or create the user data path with validation and security checks.

    Args:
        local_root_dir: Base root directory for user data

    Returns:
        Path: Validated user data directory path
    """
    local_root_sub = "dinoair"
    app_root = local_root_dir / local_root_sub

    # Check for environment variable override
    if user_data_path := os.environ.get("DINOAIR_USER_DATA"):
        user_dir = Path(user_data_path).expanduser().resolve()
        if is_path_within(local_root_dir, user_dir):
            return user_dir
        logging.warning(
            "Rejected DINOAIR_USER_DATA outside of permitted root '%s': '%s'",
            local_root_dir,
            user_dir,
        )
...
def get_default_root_dir() -> Path:
    ...
    Returns:
        Path to the default root directory based on platform
    """
    if os.name == "nt":
        return Path(DEFAULT_WINDOWS_APPDATA).expanduser().resolve()
    return Path(DEFAULT_UNIX_XDG).expanduser().resolve()


# Helper to determine if the resolved root directory is safe
def is_safe_user_root(path: Path) -> bool:
    """Check if the given path is a safe user root directory.

    Args:
        path: The path to validate

    Returns:
        True if the path is safe for user data storage, False otherwise
    """
    # Disallow system directories and root
    unsafe_windows = ["C:\\Windows", "C:\\", "C:\\ProgramData"]
    unsafe_unix = ["/", "/etc", "/bin", "/usr", tempfile.gettempdir(), "/var", "/root"]
    resolved = str(path)
    if os.name == "nt":
...
if os.name == "nt":
    ...
    xdg_env_path = os.environ.get("XDG_DATA_HOME")
    user_home = Path.home().resolve()
    default_xdg = Path("~/.local/share").expanduser().resolve()
    if xdg_env_path:
        # Normalize, absolutize and validate as string BEFORE Path object creation or resolution
        expanded_env_path = Path(xdg_env_path).expanduser()
        norm_env_path = os.path.normpath(expanded_env_path)
        abs_env_path = Path(norm_env_path).resolve()
        try:
            ...
            abs_env_path_str = str(abs_env_path)
...
class DatabaseManager:
    ...
    def __init__(
        ...
        Supports a test-friendly single-database mode when db_path is provided:
        - get_file_search_connection() will connect directly to this path
        - db_path attribute is exposed for tests
        """
        ...
        self.user_db_dir = self._determine_user_db_dir()
    ...
    @staticmethod
    def _determine_user_name(user_name: str | None) -> str:
    ...
    def _determine_base_dir(self, base_dir: Path | str | None) -> Path:
        if self._single_db_path is not None:
            return self._single_db_path.parent.resolve()
        if base_dir is not None:
            return Path(base_dir).resolve()
        return _get_default_user_data_directory()

    def _handle_pytest_redirection(self) -> None:
        try:
            if os.environ.get("PYTEST_CURRENT_TEST") and self._single_db_path is None:
    ...
    def _validate_and_set_base_dir(self) -> None:
        try:
    ...
    def _determine_user_db_dir(self) -> Path:
        if self._single_db_path is not None:
            return self._single_db_path.parent
        return self.base_dir / "user_data" / self.user_name / "databases"
    ...
    def _setup_schema(self, db_key: str, conn: sqlite3.Connection) -> None:
        ...
        if ddls := SCHEMA_DDLS.get(db_key, []):
            DatabaseManager._exec_ddl_batch(conn, ddls)
    ...
    @staticmethod
    def _validate_user_data_permissions(path: Path) -> bool:
        """
        Validate that we have proper permissions to create/write to user data directory.
    ...
    @staticmethod
    def _exec_ddl_batch(conn: sqlite3.Connection, ddl_statements: list[str]) -> None:
        """
        Execute a batch of DDL statements with proper error handling.

        Args:
            conn: Database connection
            ddl_statements: List of DDL SQL statements to execute
        """
        cursor = conn.cursor()
        try:
            ...
            conn.rollback()
            raise sqlite3.Error(f"DDL batch execution failed: {e}") from e
        finally:
            cursor.close()
    ...
    def _get_connection(self, db_key: str) -> sqlite3.Connection:
        ...
        """
        # Support single-db override for tests (file_search only)
        ...
        # Only ensure directory for real filesystem paths
        if not str(db_path).startswith("file:"):
            _ensure_dir(db_path.parent)

        def setup_callback(conn: sqlite3.Connection) -> None:
            """Setup schema for the database connection."""
            self._setup_schema(db_key, conn)

        db = ResilientDB(db_path, setup_callback, self.user_feedback)
        conn = db.connect_with_retry()
        # In single-db (test) mode, do not retain strong references to connections.
        if self._single_db_path is None:
    ...
    def initialize_all_databases(self) -> None:
        ...
        try:
            ...
            for db_key in DB_FILES:
                ...
                def setup_callback(conn: sqlite3.Connection, key: str = db_key) -> None:
                    """Setup schema for the database connection."""
                    self._setup_schema(key, conn)
    ...
    def get_watchdog_metrics_manager(self) -> WatchdogMetricsManager:
        """Get WatchdogMetricsManager instance with memory database connection"""
        # NOTE: Local import is intentional to avoid circular dependencies
        # This is a standard pattern when models depend on the database layer
        from models.watchdog_metrics import WatchdogMetricsManager  # noqa: E402
    ...
    def _cleanup_temp_files(self, stats: dict[str, int | float]) -> None:
            ...
            for temp_file in base_dir.glob(f"**/{pattern}"):
                try:
                    if not temp_file.is_file():
                        continue
                    size_mb = temp_file.stat().st_size / BYTES_TO_MB_DIVISOR
                    temp_file.unlink()
    ...
    def cleanup_user_data(
        ...
        """
        stats = {"files_removed": 0, "space_freed_mb": 0.0, "backups_removed": 0}
...
# For easy initialization when an external client boots
...
# Example usage in an external client application:
if __name__ == "__main__":
    # Test with console feedback
    def console_feedback(message: str) -> None:
        """No-op stub for testing console feedback."""
