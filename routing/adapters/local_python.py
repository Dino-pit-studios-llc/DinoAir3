...
# Allowlist of modules and their permitted callable entrypoints, derived from configs/examples.
ALLOWED_LOCAL_MODULES: dict[str, set[str]] = {
    "api.services.translator": {"router_translate"},
    "api.services.search": {"router_search"},
    # Used by lmstudio config variant
...
def _resolve_function(path: str) -> Callable[[dict[str, Any]], object]:
    ...
    # Enforce allowlist for module and function
    ...
    # Import module via safe helper using explicit mapping
    ...
    # Load attribute with per-module allowlist
    try:
