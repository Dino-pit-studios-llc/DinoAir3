"""
Pydantic-backed JSON-schema-like validation helpers for routing.
...
- Root type "object" with:
  ...
  - arrays: items.type mapped when present; default object
...
# Common type annotation constant (SonarQube S1192 compliance)
JSONValueMapping = Mapping[str, JSONValue]
DescriptorType = Union["ServiceDescriptor", JSONValueMapping, object]
...
def _array_type_from_prop(prop: JSONValueMapping) -> type[object]:
...
def _string_field_def(
    key: str, prop: JSONValueMapping, required: list[str] | None
) -> FieldDefinition:
...
def _build_model_from_schema(
    ...
    - Required keys missing in 'properties' become required object.
    """
    ...
    if isinstance(props_raw, Mapping):
        props: JSONValueMapping | None = cast(JSONValueMapping, props_raw)
    else:
        props = None
...
def validate_input(
    ...
    - On failure: raise routing.errors.ValidationError with details.
    """
...
def validate_output(
    ...
    if isinstance(payload, Mapping):
        candidate = dict(cast(JSONValueMapping, payload))
    ...
    try:
        ...
        raise ValidationError(
            ...
            details=cast("dict[str, JSONValue]", e.errors()),
        ) from e
    except Exception as e:
        raise ValidationError(
            f"output validation failed for '{name}'",
